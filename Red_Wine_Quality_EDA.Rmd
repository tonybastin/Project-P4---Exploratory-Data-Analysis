...
grid.arrange(ggplot(data = df, aes(x = sulphates, y = alcohol)) +
  geom_point()+geom_smooth(method = 'lm') + ggtitle("R-value : ",round(cor(df$sulphates, df$alcohol), 3)) ,

ggplot(data = df, aes(x = volatile.acidity, y = alcohol)) +
  geom_point()+geom_smooth(method = 'lm')+ ggtitle("R-value : ",round(cor(df$volatile.acidity, df$alcohol), 3)) ,

ggplot(data = df, aes(x = citric.acid, y = alcohol)) +
  geom_point()+geom_smooth(method = 'lm')+ ggtitle("R-value : ",round(cor(df$citric.acid, df$alcohol), 3)) ,

ggplot(data = df, aes(x = volatile.acidity, y = sulphates)) +
  geom_point()+geom_smooth(method = 'lm')+ ggtitle("R-value : ",round(cor(df$volatile.acidity, df$sulphates), 3)) ,

ggplot(data = df, aes(x = citric.acid, y = sulphates)) +
  geom_point()+geom_smooth(method = 'lm')+ ggtitle("R-value : ",round(cor(df$citric.acid, df$sulphates), 3)) ,

ggplot(data = df, aes(x = citric.acid, y = volatile.acidity)) +
  geom_point()+geom_smooth(method = 'lm')+ ggtitle("R-value : ",round(cor(df$citric.acid, df$volatile.acidity), 3)) ,
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
 ggplot(df, aes(x= quality, y=alcohol, color = volatile.acidity))+
        geom_jitter(alpha=1/4,size=3)+
        scale_colour_gradient2(low="red", high="blue",
                               mid="green",midpoint = 0.8)
...
g1 <- ggplot(data = df, aes(x = quality, y = volatile.acidity,fill = quality)) +
  ylab('Volatile Acidity') +
  xlab('Quality') +
  ggtitle('Acidity in Wines ')+
  geom_boxplot()

g2 <- ggplot(data = df, aes(x = quality, y = citric.acid,fill = quality)) +
  ylab('Citric Acid') +
  xlab('Quality') +
  geom_boxplot()
